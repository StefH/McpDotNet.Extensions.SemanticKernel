...
using var wireMockServer = await InitWireMockServerAsync(cts.Token);
...
await kernel.Plugins.AddMcpFunctionsFromSseServerAsync("GitHub", new Uri(wireMockServer.Url!), cancellationToken: cts.Token);
...
var result = await kernel.InvokePromptAsync("Which tools are currently registered?", new(executionSettings), cancellationToken: cts.Token);
Console.WriteLine($"\n\nTools:\n{result}");

var promptGitHub = "Summarize the last commit to the StefH/FluentBuilder repository";
var resultGitHub = await kernel.InvokePromptAsync(promptGitHub, new(executionSettings), cancellationToken: cts.Token);
Console.WriteLine($"\n\n{promptGitHub}\n{resultGitHub}");

return;

static async Task<WireMockServer> InitWireMockServerAsync(CancellationToken cancellationToken)
{
