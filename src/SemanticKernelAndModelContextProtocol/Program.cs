...
using SemanticKernelAndModelContextProtocol;
using WireMock.Server;

await GetGitHubToolsAsync();
return;

static async Task GetGitHubToolsAsync()
{
    ...
    var toolsResponse =
        """
        {"result":{"tools":[{"name":"list_commits","description":"Get list of commits of a branch in a GitHub repository","inputSchema":{"type":"object","properties":{"owner":{"type":"string"},"repo":{"type":"string"},"sha":{"type":"string"},"page":{"type":"number"},"perPage":{"type":"number"}},"required":["owner","repo"],"additionalProperties":false,"$schema":"http://json-schema.org/draft-07/schema#"}}]},"jsonrpc":"2.0","id":3}
        """;
    ...
    var mcpClient = await factory.GetClientAsync(config.Id);
    ...
    var prompt = "Summarize the last commits to the StefH/FluentBuilder repository?";
    var result = await kernel.InvokePromptAsync(prompt, new(executionSettings)).ConfigureAwait(false);
    Console.WriteLine($"\n\n{prompt}\n{result}");

    var le = server.LogEntries;
    var p = le.Where(l => l.RequestMessage.Method == "POST").ToArray();
}