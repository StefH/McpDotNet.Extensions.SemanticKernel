...
// Copied from JsonConverter.Newtonsoft.Json
internal static class DynamicClassFactory
{
    ...
    private static readonly MethodInfo EqualityComparerGetHashCode = EqualityComparer.GetMethod("GetHashCode", [EqualityComparerGenericArgument])!;
#else
    ...
    private static readonly MethodInfo EqualityComparerEquals = EqualityComparer.GetMethod("Equals", BindingFlags.Instance | BindingFlags.Public, null, [EqualityComparerGenericArgument, EqualityComparerGenericArgument], null)!;
    private static readonly MethodInfo EqualityComparerGetHashCode = EqualityComparer.GetMethod("GetHashCode", BindingFlags.Instance | BindingFlags.Public, null, [EqualityComparerGenericArgument], null)!;
#endif

    private static int _index = -1;

    private const string DynamicAssemblyName = "Stef.ModelContextProtocol.SemanticKernel.Dynamic, Version=1.0.0.0";
    private const string DynamicModuleName = "Stef.ModelContextProtocol.SemanticKernel.Dynamic";
        ...
        if (!GeneratedTypes.TryGetValue(key, out var type))
        {
            ...
            lock (GeneratedTypes)
            {
                if (!GeneratedTypes.TryGetValue(key, out type))
                {
                    ...
                    // Equals
                    MethodBuilder equals = tb.DefineMethod("Equals", MethodAttributes.Public | MethodAttributes.Virtual | MethodAttributes.HideBySig, CallingConventions.HasThis, typeof(bool), [typeof(object)]);
                    ...
                    if (createParameterCtor && names.Length > 0)
                    {
                        // .ctor default
                    ...
                    type = GeneratedTypes.GetOrAdd(key, tb.CreateTypeInfo()!.AsType()!);
                }
            }
        }
